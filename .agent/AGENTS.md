...
* 你正在协助的对象是 **Chase**。
*  Chase 是一名经验丰富的资深后端 / 数据库工程师，熟悉 Python 等主流语言及其生态。
...
* 你的核心目标：
  ...
  * 当你需要给出方案或者用户向你询问建议的时候，你需要使用context7 mcp获取如Dify，Alibaba，Manus，Google，Microsoft等专业团队的最佳实践。
  * 命令行运行环境是Windows PowerShell，请注意路径分隔符的使用。
  * 编辑文件的时候不要改变文件的编码格式，保持UTF-8编码
  * 当出现报错信息的时候，不要把提示关闭，或者简单的禁止，而是研究问题的本质，从根本上解决问题
  * 完成一个active任务之后把任务放到completed文件夹
  * 执行任何一个计划，完成代码编写之后，都需要进入检查修复循环，检查：首先检查代码改动是否符合计划要求，是否实现了计划提到的所有内容，然后检查代码改动本身是否有问题，修复：修复检查阶段发现的所有问题。退出检查修复循环的标准是你认为没有任何问题了。
  * docs/dev文档只是作为一个计划记录，不是系统当前状态，不能认为是系统当前状态，不需要阅读这个文档
  ...
  * plan的phasex需要有明确的定义和边界，且phase需要递进，在边界里面的添加到这个phase，不在这个边界里面的重新开一个phase
---
...
* 严格遵循各语言社区的惯用写法与最佳实践（TypeScript等）。
* 主动留意并指出以下“坏味道”：
  * 重复逻辑 / 复制粘贴代码；
  * 模块间耦合过紧或循环依赖；
  * 改动一处导致大量无关部分破坏的脆弱设计；
...
4. 在 **缺失信息会阻碍继续推进或改变方案选择** 时，提出澄清问题；
   * 若不得不做假设，需显式说明关键假设。
